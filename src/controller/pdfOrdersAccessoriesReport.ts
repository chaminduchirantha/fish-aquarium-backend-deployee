...
export const generateAccessoriesOrderReport = async (req: Request, res: Response) => {
  try {
    const orders = await OrdersAccessories.find().sort({ createdAt: -1 });

    const reportDir = path.join(__dirname, "../reports");
    if (!fs.existsSync(reportDir)) fs.mkdirSync(reportDir, { recursive: true });

    const filePath = path.join(reportDir, "orders-clean-report-2.pdf");
    ...
    doc.fontSize(22).font("Helvetica-Bold").fillColor("#003566")
      .text("Accessories Orders Report", { align: "center" });
    doc.moveDown(2);
    ...
    orders.forEach((order: any, index: number) => {
      ...
      doc.text(`Fish: ${order.itemname}`, 40, startY + 65);
